...
select tb_produtos.nome, tb_usuarios.nome as vendedor -- muda de 'nome' para 'vendedor' 
from tb_produtos inner join tb_usuarios
on tb_usuarios.id = tb_produtos.usuario_id;

select tb_produtos.nome,
concat('R$ ', format(tb_produtos.preco, 2, 'pt_BR')) as preco, -- formatando para moeda do brasil
tb_categorias.tipo, tb_usuarios.nome as vendedor
from tb_produtos
inner join tb_categorias on tb_categorias.id = tb_produtos.categoria_id
inner join tb_usuarios on tb_usuarios.id = tb_produtos.usuario_id; -- mais de um inner join
